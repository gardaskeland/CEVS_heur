...
bool cmp_ascending = [](pair<int, int> &left, pair<int, int> &right) { return left.first < right.first; };
...
int remove_3(Graph &g, SolutionRepresentation &sol, tri &t, Modifications &mod) {
    int cost = 0;
    set<int> neighbours;
...
//Removed contains node and as set it is removed from
...
int add_3(Graph &g, SolutionRepresentation &sol, tri &t, Modifications &mod) {
