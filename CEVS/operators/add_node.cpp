...
optional<int> add_node_to_set_unchanged(Graph &g, SolutionRepresentation &sol) {
    set<int> changed = sol.book.modified_clusters.query(max(0, sol.book.operation_number - 5*g.n), sol.book.operation_number - 1);
    vector<int> choices;
...
optional<int> add_node_to_neighbours_accept_unchanged(Graph &g, SolutionRepresentation &sol, int i) {
    set<int> changed_nodes = sol.book.modified_vertices.query(max(0, i - 5*g.n), i);
...
optional<int> remove_node_accept_unchanged(Graph &g, SolutionRepresentation &sol, int i) {
    set<int> changed = sol.book.modified_vertices.query(max(0, i - 5*g.n), i);
    vector<int> unchanged;
    for (int j = 0; j < g.n; j++) {
